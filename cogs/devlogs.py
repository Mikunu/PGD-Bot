...
from extensions.useful_funtions import check_if_user_has_projects, grant_channel_roles, is_in_devlogs, \
    check_if_space_in_category_exists


class Devlogs(commands.Cog, description='Команды девлогов'):
    ...
    async def create(self, ctx: discord.ext.commands.Context, member: discord.Member):
        if member is None:
            await ctx.reply('Необходимо указать пользователя, которому создастся канал')
            return
        ...
        if check_if_space_in_category_exists(ctx):
            await ctx.reply('Место в категории проектов закончилось')
            return
    ...
    @devlog.command(name='rename-project',
    ...
    @devlog.command(name='update-topic', rest_is_raw=True,
    ...
    @devlog.command(name='projects-remove2fa')
    ...
    @devlog.command(name='projects-get2fa')
